...
(function () {
  ...
  function populateFooter() {
    ...
    if (socialContainer && site?.social) {
      ...
      socialContainer.innerHTML = [
        ...
        .map(
          ([key, v]) => {
            const iconAsset = getSocialAsset(key);
            const iconHtml = iconAsset
              ? `<img src="${iconAsset}" alt="" class="footer-social-img" width="18" height="18" loading="lazy">`
              : `<svg fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">${getSocialSvg(key)}</svg>`;
            return `<a href="${v.url}" aria-label="${v.label}" data-social="${key}">${iconHtml}</a>`;
          }
        )
        .join('');
    }
  ...
  function getSocialSvg(key) {
    const svgs = {
      ...
      googleMaps:
        '<path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z"></path><circle cx="12" cy="9" r="2.5" fill="white"></circle>',
      instagram:
  ...
  function getSocialAsset(key) {
    const assets = {
      instagram: 'imgs/social-instagram.svg',
      linkedin: 'imgs/social-linkedin.svg',
    };
    return assets[key] || '';
  }
